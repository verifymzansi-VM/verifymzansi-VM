...
const log = createLogger("PromotionUrgentCheckout");
const promotionUrgentParamsSchema = z.object({
  id: uuidSchema,
});
...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsedParams = parseAndValidateRouteParams(await params, promotionUrgentParamsSchema, {
    ...
    if (rateCheck.limited) {
      ...
      return NextResponse.json(
        { error: "Too many promotion urgent attempts. Please try again later." },
    ...
    const { data: rawPromotion } = await applyOwnerFilter(
      supabase
        .from("promotions")
        .select(withOwnerColumn("id, title, status, owner_id, urgent_until", ownerColumn))
        .eq("id", promotionId),
      ownerColumn,
      user.id
    ).maybeSingle();
    ...
    if (promotion.status !== "live") {
      return NextResponse.json(
        { error: "Only live promotions can be marked urgent" },
        { status: 400 }
      );
    }

    if (promotion.urgent_until && new Date(promotion.urgent_until) > new Date()) {
      return NextResponse.json(
        { error: "This promotion is already marked urgent" },
        { status: 400 }
      );
    }
    ...
    const { data: pendingPmt, error: pendingError } = await admin
      ...
      .contains("provider_data", { type: "urgent_promotion", promotion_id: promotionId })
      .maybeSingle();
    ...
    if (pendingPmt) {
      return NextResponse.json(
        { error: "An urgent payment is already in progress for this promotion" },
        { status: 409 }
      );
    }
    ...
    const { paymentId, checkoutUrl } = await createHostedCheckout({
      ...
      itemName: `Urgent: ${promotion.title}`.slice(0, 100),
