...
const promotionFeaturedParamsSchema = z.object({
  id: uuidSchema,
});
...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsedParams = parseAndValidateRouteParams(await params, promotionFeaturedParamsSchema, {
    ...
    if (rateCheck.limited) {
      ...
      return NextResponse.json(
        { error: "Too many promotion feature attempts. Please try again later." },
    ...
    const { data: pendingPmt } = await admin
      ...
      .contains("provider_data", { type: "featured_promotion", promotion_id: promotionId })
      .maybeSingle();
    if (pendingPmt) {
      return NextResponse.json(
        { error: "A featured payment is already in progress for this promotion" },
        { status: 409 }
      );
    }
