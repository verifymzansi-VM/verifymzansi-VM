...
function ShowroomProgressDots({
  ...
  if (count <= 1) return <div className="showroom-progress-track h-[52px]" aria-hidden="true" />;
  const { start, end } = getVisibleDotRange(count, activeIndex);
  const indexes = Array.from({ length: end - start + 1 }, (_, i) => start + i);

  return (
    <div
      className="showroom-progress-track relative z-20 flex h-[52px] items-center justify-center gap-1 pb-1 pt-3"
  ...
  if (count === 0) {
    return (
      ...
      >
        ...
        <div className="showroom-progress-track h-[52px]" aria-hidden="true" />
      </SectionShell>
    );
  }
