...
import { POST } from "@/app/api/business-ads/[id]/boost/route";
...
describe("POST /api/business-ads/[id]/boost", () => {
  beforeEach(() => vi.clearAllMocks());

  it("rejects invalid UUID", async () => {
    mockAuth({ id: USER_ID });
    const req = createRequest("http://localhost:3000/api/business-ads/not-a-uuid/boost");
  ...
  it("rejects unauthenticated requests", async () => {
    mockAuth(null);
    const req = createRequest(`http://localhost:3000/api/business-ads/${VALID_UUID}/boost`);
  ...
  it("returns 404 when business profile not found", async () => {
    ...
    expect(res.status).toBe(404);
    const body = await res.json();
    expect(body.error).toBe("Business profile not found");
  });
  ...
  it("returns 400 when profile is not live", async () => {
    mockAuth({ id: USER_ID });
    mockAdmin({
      seller_profiles: {
        maybeSingle: vi.fn().mockResolvedValue({ data: { id: "sp-1" } }),
  ...
  it("returns 400 when profile is already boosted", async () => {
    mockAuth({ id: USER_ID });
  ...
  it("logs audit event on successful checkout", async () => {
    ...
    expect(auditEntry.targetType).toBe("business_profile");
  });
});
