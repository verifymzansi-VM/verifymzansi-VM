...
import { POST } from "@/app/api/storefronts/[id]/boost/route";
...
function setupHappyPath() {
  mockAuth({ id: USER_ID, app_metadata: {} });
...
describe("POST /api/storefronts/[id]/boost", () => {
  beforeEach(() => vi.clearAllMocks());

  it("rejects invalid UUID", async () => {
    mockAuth({ id: USER_ID });
    const req = createRequest("http://localhost:3000/api/storefronts/not-a-uuid/boost");
  ...
  it("rejects unauthenticated requests", async () => {
    mockAuth(null);
    const req = createRequest(`http://localhost:3000/api/storefronts/${VALID_UUID}/boost`);
  ...
  it("returns 404 when storefront not found", async () => {
    mockAuth({ id: USER_ID });
    mockAdmin({
      seller_profiles: {
        maybeSingle: vi.fn().mockResolvedValue({ data: { id: "sp-1" } }),
    ...
    expect(res.status).toBe(404);
    const body = await res.json();
    expect(body.error).toBe("Storefront not found");
  });

  it("returns 403 when user does not own the storefront", async () => {
    mockAuth({ id: USER_ID });
    mockAdmin({
      seller_profiles: {
        maybeSingle: vi.fn().mockResolvedValue({ data: { id: "sp-1" } }),
  ...
  it("returns 400 when storefront is not live", async () => {
    mockAuth({ id: USER_ID });
    mockAdmin({
      seller_profiles: {
        maybeSingle: vi.fn().mockResolvedValue({ data: { id: "sp-1" } }),
  ...
  it("returns 400 when storefront is already boosted", async () => {
    mockAuth({ id: USER_ID });
  ...
  it("returns 403 when entitlement check fails", async () => {
    setupHappyPath();
    mockCanBoost.mockReturnValue({ allowed: false, reason: "Upgrade required" });
    const req = createRequest(`http://localhost:3000/api/storefronts/${VALID_UUID}/boost`);
    const res = await POST(req, { params: Promise.resolve({ id: VALID_UUID }) });
    expect(res.status).toBe(403);
    const body = await res.json();
  ...
  it("logs audit event on successful checkout", async () => {
    ...
    expect(auditEntry.targetType).toBe("storefront");
  });
});
