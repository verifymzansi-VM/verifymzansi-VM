...
SELECT
  ...
  bp.cover_photo,
  bp.cover_video,
  to_jsonb(bp)->>'video_thumbnail',
  COALESCE(to_jsonb(bp)->>'location_province', ''),
  COALESCE(to_jsonb(bp)->>'location_city', ''),
  bp.phone,
  bp.whatsapp,
  bp.email,
  bp.website,
  to_jsonb(bp)->'social_links',
  bp.services_offered,
  bp.service_areas,
  bp.operating_hours,
  bp.status,
